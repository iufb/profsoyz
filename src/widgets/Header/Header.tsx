...
export const Header = () => {
  return (
    <header className="h-auto relative   bg-cyan-500 flex flex-col gap-2   w-full  ">
      <section className="h-14  mr-0  md:pr-10 flex gap-6 items-center md:static fixed bg-white z-50 left-0 right-0 top-0  justify-end pt-3 pr-3">
        <Links />
        <SocialLinks />
        <ChangeLocale />
        <BurgerMenu />
      </section>
      <section className="min-h-[100px] md:min-h-[130px] grid-cols-1  before:w-full  before:h-2 md:before:h-5  relative before:z-50 before:absolute before:left-0 before:right-0 before:bg-navbar-texture before:bg-repeat-x before:-bottom-2  md:before:-bottom-5 before:text-cyan-500 before:rotate-180     md:px-20 px-5  grid mt-14 md:mt-0 md:grid-cols-[100px_1fr]  items-center gap-5">
        <Image
          src={"/logo.svg"}
          height={80}
          width={80}
...
const Links = () => {
  return (
    <div className="text-cyan-500 gap-10  hidden lg:flex">
